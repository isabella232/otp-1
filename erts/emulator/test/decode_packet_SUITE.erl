...
-export([all/0, suite/0,groups/0,
         ...
         otp_9389/1, otp_9389_line/1, match_spec/1]).
...
all() ->
    [basic, packet_size, neg, http, line, ssl, otp_8536,
     otp_9389, otp_9389_line, match_spec].
...
match_spec(Config) when is_list(Config) ->
    GenSpec = fun() ->
                      Types = [u8, varint, u16, u16le, u32, u32le],
                      %% Max match_specs is 10
    ...
    TestSpec = fun(Spec) ->
                       ...
                       {ok, Packet, DataRest} = erlang:decode_packet(match_spec,
